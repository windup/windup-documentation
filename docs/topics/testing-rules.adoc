...
In addition, it is recommended to create a test rule that follows the name of the rule it tests. For instance, if a rule were created with a filename of `proprietary-rule.mtr.xml`, the test rule should be called `proprietary-rule.windup.test.xml`.
