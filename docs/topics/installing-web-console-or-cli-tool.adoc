...
ifdef::cli-guide[]
. Navigate to the link:{MTRDownloadPageURL}[{ProductShortName} Download page] and download the `Migration Toolkit CLI` file.
endif::[]
ifdef::web-console-guide[]
...
. Extract the `.zip` file to a folder named `mtr` to avoid a `Path too long` error. Alternatively, extract the file with link:https://www.7-zip.org/download.html[7-Zip] to a folder of any name you choose.
...
----
+
. Open a browser and navigate to `\http://localhost:8080/windup_ui`.
+
