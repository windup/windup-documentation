...
. Replaces each `xref:` to external pages with an `xref:` in the docs/topics/ folder using the following command.
...
+
----
mkdir -p docs/2.5.0.Final/html/images (if it doesn't exist!)
----
    
* Copy any the images, stylesheets, and html files from windup-documentation:
+
----
cp windup-documentation/html/*.html windup/docs/2.5.0.Final/html/
cp windup-documentation/*.css windup/docs/2.5.0.Final/html/
cp windup-documentation/images/* windup/docs/2.5.0.Final/html/images/
----

* Copy the JavaDoc files that were backed up from `target/apidocs` to a javadoc folder:
+
----
cp -r temp/target/apidocs windup/docs/2.5.0.Final/javadocs/
----
...
** Create a symlink to the new documentation.
        ...
        Example: ln -s 2.5.0.Final latest
...
** Commit the change.
        ...
        git commit -m 'Replace symlink for latest to point to 2.5.0.Final'
