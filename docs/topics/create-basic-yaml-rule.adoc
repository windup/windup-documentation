...
+
Use the `--rules` option in the CLI.
+
[source,terminal]
----
...
----
+
Note that if you wish to use the `--target` or `--source` option in the CLI, the engine will only select rules that match the label for that target. Therefore, ensure that you have added target or source labels on your rules. See Reserved labels for more details.
