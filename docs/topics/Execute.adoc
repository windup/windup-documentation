...
Right now only XML validations rely on Internet access - so in offline mode XML files won't be validated. 
...
--skipReports:: If set, Windup will not generate reports. This is an advanced option and is not recommended to use by "standard" end user.
