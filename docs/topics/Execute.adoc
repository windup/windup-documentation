...
+
The `--source` argument now supports the link:http://maven.apache.org/enforcer/enforcer-rules/versionRanges.html[Maven version range syntax], which allows you to run the rules appropriate for your specific technology version. For example, `--source eap:5`.
+
[WARNING]
====
When migrating from an earlier JBoss EAP version to a newer one, be sure to specify the version for the source and target, for example, `eap:6`. Specifying only `eap` may run both JBoss EAP 6 and JBoss EAP 7 rulesets, which can cause duplicate hits.
...
+
The `--target` argument now supports the link:http://maven.apache.org/enforcer/enforcer-rules/versionRanges.html[Maven version range syntax], which allows you to run the rules appropriate for your specific technology version. For example, `--target eap:7`.
+
[WARNING]
====
When migrating to JBoss EAP, be sure to specify the version in the target, for example, `eap:6`. Specifying only `eap` may run both JBoss EAP 6 and JBoss EAP 7 rulesets, which can cause duplicate hits.
