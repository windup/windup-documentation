...
[WARNING]
====
When migrating from an earlier JBoss EAP version to a newer one, be sure to specify the version for the source and target, for example, `eap:6`. Specifying only `eap` will run rulesets for all versions of JBoss EAP, including those not relevant to your migration path.
