...
Every custom questionnaire field marked as *required* is mandatory and must be completed. Otherwise, the YAML syntax will not validate on upload. Each subsection of the field defines a new struct/object in YAML, for example:
...
[options="header"]
|====
|Questionnaire filed|Description
...
* `order` (required, int): An order of the question in the section.
