...
. Extract the `.zip` file to a directory, for example, `MTR_HOME`.
endif::[]
ifdef::ocp-46[]
...
. Copy the contents of the appropriate template from the `MTR_HOME/openshift/templates/` directory into the *Import YAML* field.
+
