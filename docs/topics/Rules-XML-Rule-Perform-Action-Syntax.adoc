...
|category-id
|STRING
a|A reference to a category as defined in `WINDUP_HOME/rules/migration-core/core.windup.categories.xml`. The default categories are `mandatory`, `optional`, and `potential`.
...
|<quickfix>
a|Contains information to be used by the {ProductPluginName} to perform quick fixes when the rule condition is met.
