...
|category-id
|STRING
a|A reference to a category as defined in `MTR_HOME/rules/migration-core/core.windup.categories.xml`. The default categories are `mandatory`, `optional`, `potential`, and `information`.
