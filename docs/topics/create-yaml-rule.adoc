...
|`hasTags`
3+>|This is an inline list of string tags. See xref:yaml-rule-actions_rule-development-guide[Tag actions] for details on tag format.
...
|===
+
You can use the following search locations to scope down `java` searches:
+
* CONSTRUCTOR_CALL
* TYPE
* INHERITANCE
...
+
The following example shows the `when` condition of a rule that searches for references of a package.
+
_Example_
+
[source,terminal]
