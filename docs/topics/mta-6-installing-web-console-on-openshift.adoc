...
{ProductShortName} creates a basic
https://access.redhat.com/documentation/en-us/red_hat_single_sign-on/7.6[Red
Hat Single Sign-On] instance, using the Keycloak CR of Red Hat Single
Sign-On Operator. Keycloak is a single sign-on solution for web apps and
RESTful web services.

The {ProductShortName} Keycloak realm is fully configured by the Hub.

{ProductShortName} does not enable RHSSO external access by default. However, if you
require RHSSO, you can configure this by using the Tackle CR attribute
`rhsso_external_access`. Otherwise, use the {ProductShortName} realm admin console to
adjust settings.
...
The following table contains the roles and permissions (scopes) {ProductShortName}
seeds RHSSO with:
