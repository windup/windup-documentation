...
A node running applications requires sufficient reserve memory to operate successfully. Whenever an application uses more than its reserved memory allocation, it can trigger an out-of-memory event.

The `--eviction-hard` flag directs the node to evict applications when available memory drops below an absolute value or percentage value.

Set `--eviction-hard` flag along with memory reserves for the node.
...
This node has 28.9 GiB of [effective] memory available (32 GiB - 3 GiB - 0.1 GiB) before the `--eviction-hard` flag starts to evict applications that exceed the recommended memory requirements. 
