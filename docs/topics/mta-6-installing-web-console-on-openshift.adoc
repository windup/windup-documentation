...
A node running applications requires enough reserve memory. Whenever an application uses more than its reserved memory, it can trigger an out-of-memory event.
...
This node has 28.9 Gi of effective memory available (32 GiB - 3 GiB - 0.1 GiB) before the `--eviction-hard` setting starts to evict applications that are using too much memory.
