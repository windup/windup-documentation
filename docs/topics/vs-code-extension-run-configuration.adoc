...
* `{LC_PSN}-cli` executable installed. You can download the `{LC_PSN}-cli` executable from link:https://developers.redhat.com/products/{LC_PSN}/download[{ProductShortName} download].
...
** *`{LC_PSN}-cli`*: Enter the path to the `{LC_PSN}-cli` executable. For example: `$HOME/mtr-cli-1.0.0.GA-redhat-00001/bin/mtr-cli`.
