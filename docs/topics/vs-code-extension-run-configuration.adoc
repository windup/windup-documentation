...
* `{LC_PSN}-cli` executable installed. You can download the `{LC_PSN}-cli` executable from link:https://developers.redhat.com{LC_PSN}/ducts/{LC_PSN}/download[{ProductShortName} download].

.Procedure

. In *Extensions* view, click the {ProductName} icon (image:vs_{ProductShortName}_extension_icon.png[{ProductShortName} code extension]) on the Activity bar.
...
** *`{LC_PSN}-cli`*: Enter the path to the `{LC_PSN}-cli` executable. For example: `$HOME/mta-cli-5.3.0.Final/bin/mta-cli`.
