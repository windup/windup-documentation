...
ifdef::disconnected[]
. On a computer with network access, navigate to the {ProductName} link:{{ProductShortName}DownloadPageURL}[download site] and download the `{IDEPluginFilename}` file.
endif::[]
