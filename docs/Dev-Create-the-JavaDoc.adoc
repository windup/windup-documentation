...
If you want to build an updated version, use the following procedure to create JavaDoc for {ProductName}.

. In {ProductName} project root directory, type the following Maven command:
+
-----------------------------------
mvn validate -PjavadocDist
-----------------------------------
. The JavaDoc is created in the {ProductName} `target/apidocs` subdirectory.
