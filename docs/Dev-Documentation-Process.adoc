...
The https://github.com/windup/windup-documentation/tree/master/scripts/windupDocUpdate.sh script copies the pages from the {ProductName} wiki to the windup-documentation repository, makes minor modifications, and generates each guide in a single HTML page book format. The resulting guides are created in the `{productdochomevar}/html/` directory.

The script performs the following steps.

. Copies the {ProductName} wiki pages to the `{productdochomevar}/docs/` directory, renaming them with the `.adoc` extension.

. Makes sure each guide includes all pages that are referenced by pages within it. Scripts are provided to search for links within each individual guide. They navigate to the `{productdochomevar}/docs/` directory and search using the following command:
...
. Navigates to the `{productdochomevar}/` directory and builds the books. The single page HTML files are created in the `{productdochomevar}/html/` directory.
...
The https://github.com/windup/windup-documentation/tree/master/scripts/windupPublish.sh script copies the pages from the {ProductName} wiki to the windup-documentation repository, makes minor modifications, and generates each guide in a single HTML page book format. The resulting guides are created in the `{productsrchomevar}/html/` directory.
...
* Navigate to the local `{productsrchomevar}/` directory.
+
----
cd windup
----
...
* If the `{productsrchomevar}/docs/{productreleasevar}/` directory does not yet exist, create a `docs/{productreleasevar}/html/images/` 
+
----
...
cp {productdochome}/html/*.html {productsrchome}/docs/{productreleasevar}/html/
cp {productdochomevar}/*.css {productsrchomevar}/docs/{productrelease}/html/
cp {productdochomevar}/images/* {productsrchomevar}/docs/{productreleasevar}/html/images/
