...
The upstream frames project only stores one type in the _type_ property by default. {ProductName} extends this default type resolver behavior to support multiple types. The values themselves are stored in a multi-valued Titan property on each Vertex.
