...
If you plan to contribute to the core code base or create Java-based rule add-ons, you must first install and configure Maven to build {ProductName} from source.

==== Download and Install Maven

If you plan to use Eclipse Luna (4.4) to build {ProductName}, you can skip this
step and go directly to the section entitled xref:configure-maven-to-build-windup[Configure Maven to Build {ProductName}]. This version of Eclipse embeds Maven 3.2.1 so you do not need to
...
==== Configure Maven to Build {ProductName}

{ProductName} uses artifacts located in the
...
You are now configured to build {ProductName}.
