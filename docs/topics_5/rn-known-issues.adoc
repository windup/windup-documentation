...
|OpenShift
|{ProductShortName} installation on OpenShift Container Platform fails. The Webconsole and Executor Pods are not running.
...
|Technical Debt
|{ProductShortName} does not run on Zulu.

|link:https://issues.redhat.com/browse/WINDUP-2502[WINDUP-2502]
|Web UI & Windup-as-a-Service
|Global custom rules appeared in the Project level *Select Custom Rules* screen.
