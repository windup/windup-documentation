...
[id='test_xml_rule_{context}']
= Testing XML Rules

== Create a Test Rule

Once a rule has been created, it is recommended to create a test rule to ensure the functionality meets the desired expectation. Test rules are created using a process similar to the process for creating a test rule, with the following differences:
...
[id='test_xml_rule_structure_{context}']
...
[id='iterable_filter_syntax_{context}']
...
[id='hint_exists_syntax_{context}']
==== <hint-exists> Syntax
