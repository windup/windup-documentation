...
Wnen you run the `windup-migrate-app` command against your application, {ProductShortName} executes its own core rules to extract files from archives, decompile classes, and analyze the application. In this phase {ProductShortName} builds a data model and stores component data and relationships in a graph database, which can then be queried and updated as needed by the migration rules and for reporting purposes.
